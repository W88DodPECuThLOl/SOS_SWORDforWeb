...
Counter::Counter(u8 channel, Counter* chain, void* userData)
	: channel(channel)
	, chain(chain)
	, userData(userData)
...
Cat8253::Cat8253(void* userData)
	: userData(userData)
{
	counters[2] = new Counter(2, nullptr, userData);
	counters[1] = new Counter(1, counters[2], userData);
	counters[0] = new Counter(0, nullptr, userData);
	reset();
}
...
void
Cat8253::adjustTick(s32& tick)
{
	// @todo
	//counters[0]->adjustTick(tick);
	//counters[1]->adjustTick(tick);
}

void
Cat8253::tick(s32 tick)
{
	// ch0 : 894.88625kHz
	// ch1 :  15.7kHz
...
void
Cat8253::setGate(const u8 address, bool isGate)
{
	counters[address]->setGate(isGate);
}

void
Cat8253::setGateDirect(const u8 address, bool isGate)
{
	counters[address]->setGateDirect(isGate);
}

void
Cat8253::write(const u32 address, const u8 value) noexcept
{
	if(address < 3) {
...
u8
Cat8253::read(const u32 address) noexcept
{
	if(address < 3) {
